<template>
  <div class="header flex items-center justify-between drag h-14">
    <div class="flex items-center pl-5" v-bind:class="{'w-100': isMobile}">
...
<!--      <IconPark :icon="Right" :size="iconSize" :stroke-width="2" class="icon-button"/>-->
      <div class="search no-drag" v-bind:class="{'search-mobile': isMobile}">
        <SearchPop/>
      </div>
    </div>
...
<!--      <IconPark :icon="Platte" :stroke-width="2" class="icon-button" @click="changeThemeMode"/>-->
      <IconPark v-if="isMobile" @click="displayMenu = !displayMenu" :icon="HamburgerButton" :stroke-width="2" class="icon-button"/>
    </div>
  </div>
</template>
...
const {displayMenu, isMobile} = storeToRefs(useMenuStore());
